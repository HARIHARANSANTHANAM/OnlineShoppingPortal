...
function CartPage(props) {
  ...
  const handleRemoveFromCart=(pro)=>{
    ...
    const dummyproduct=product;
     let updatedproduct=dummyproduct.filter((p,i)=>p.imgUrl!==pro.imgUrl);
    console.log(updatedproduct.map(d=> d["user"]=JSON.parse(localStorage.getItem("User"))));
  ...
  return (
    <div className="container">
      ...
      product[0]?
        ...
        </table>:<center><h3>Cart is Empty</h3></center>
        }
        <ToastContainer/>
      </div>
